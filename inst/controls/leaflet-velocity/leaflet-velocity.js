...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  // QUICKMAP PATCH (item 7): update wind without the restart that setData does
  updateData: function updateData(data) {
    this.options.data = data;
    if (this._windy && this._windy.updateData && this._windy.updateData(data)) {
      this.fire('load');
      return;
    }
    this.setData(data);
...
var Windy = function Windy(params) {
  ...
  var interpolateField = function interpolateField(grid, bounds, extent, callback) {
    ...
    // QUICKMAP PATCH (item 7 S2): while doing the full geometry pass, cache
    // per-cell lat/lng and the (wind-independent) distortion matrix so later
    // wind frames can rebuild the field in milliseconds (fastInterpolateField)
    var cache = [];
      ...
      for (var y = bounds.y; y <= bounds.yMax; y += 2) {
        ...
        if (coord) {
          ...
          if (isFinite(λ)) {
            ...
            cache.push(x, y, λ, φ,
              d[0] * velocityScale, d[1] * velocityScale,
              d[2] * velocityScale, d[3] * velocityScale);
            var wind = grid.interpolate(λ, φ);

            if (wind) {
              var u = wind[0] * velocityScale,
                  v = wind[1] * velocityScale;
              column[y + 1] = column[y] =
                [d[0] * u + d[2] * v, d[1] * u + d[3] * v, wind[2]];
            }
          }
        }
      }
    ...
    (function batchInterpolate() {
      ...
      geomCache = { bounds: bounds, cells: cache }; // QUICKMAP PATCH (item 7 S2)
      createField(columns, bounds, callback);
    })();
  };

  // QUICKMAP PATCH (item 7 S2): rebuild the field for a new wind frame from
  // the cached geometry — no Leaflet projection calls, so a few ms per step
  // for any grid resolution
  var fastInterpolateField = function fastInterpolateField(grid, callback) {
    var cells = geomCache.cells;
    var columns = [];

    for (var k = 0; k < cells.length; k += 8) {
      var x = cells[k];
      var column = columns[x] || (columns[x + 1] = columns[x] = []);
      var wind = grid.interpolate(cells[k + 2], cells[k + 3]);
      ...
      if (wind) {
        ...
        var u = wind[0],
            v = wind[1];
        column[y + 1] = column[y] =
          [cells[k + 4] * u + cells[k + 6] * v,
           cells[k + 5] * u + cells[k + 7] * v, wind[2]];
      }
    }

    createField(columns, geomCache.bounds, callback);
  };
  ...
  var animate = function animate(bounds, field) {
    ...
    // QUICKMAP PATCH (item 7): read the live field each tick so updateData()
    // can swap wind data without destroying the particle population
    var liveField = function () {
      return windy.field || field;
    };
    ...
    function evolve() {
      ...
      particles.forEach(function (particle) {
        if (particle.age > MAX_PARTICLE_AGE) {
          liveField().randomize(particle).age = 0;
        }

        var x = particle.x;
        var y = particle.y;
        var v = liveField()(x, y); // vector at current position
  ...
  // QUICKMAP PATCH (item 7): bounds retained for in-place data updates;
  // geomCache rebuilt by every full interpolateField run (start/zoom/pan)
  var lastLayerBounds, lastMapBounds, geomCache;
  ...
  var start = function start(bounds, width, height, extent) {
    ...
    lastLayerBounds = buildBounds(bounds, width, height); // QUICKMAP PATCH
    lastMapBounds = mapBounds; // QUICKMAP PATCH
    geomCache = null; // QUICKMAP PATCH: view changed, geometry cache is stale

    buildGrid(gridData, function (grid) {
      // interpolateField
      interpolateField(grid, lastLayerBounds, lastMapBounds, function (bounds, field) {
  ...
  // QUICKMAP PATCH (item 7): swap wind data in place — recompute the field
  // and hand it to the running animation without reseeding particles
  var updateData = function updateData(data) {
    gridData = data;
    if (!windy.field || !lastLayerBounds) return false;
    // geometry cache still building (initial start in progress): the frame is
    // stored in gridData and the next update lands on the fast path
    if (!geomCache) return true;
    buildGrid(gridData, function (grid) {
      fastInterpolateField(grid, function (bounds, field) {
        var old = windy.field;
        windy.field = field;
        if (old) old.release();
      });
