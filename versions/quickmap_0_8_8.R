...
# Version 0.8.8 - Boundary Labels Control (Critical-1)
...
#   v0.8.7 - Banner and Legend System Unification (Task 1E):
#             - Extended apply_custom_layout_in_html() to support static image mode
#             - Added image_mode parameter with image-optimized CSS for JPG clarity
#             - Added image_dimensions parameter with automatic scaling for different sizes
#             - Unified static and interactive map banner/legend processing
#             - Modified static map generation to use HTML post-processing
#             - Simplified add_map_controls() by removing duplicate legend code for static maps
#             - Eliminated inconsistency between interactive and static map styling
#             - Added comprehensive test suite for validation of all image dimensions
#   v0.8.7.1 - Banner and Legend Scaling Fixes (Task 1E.1):
#             - Improved scale factor calculation using geometric mean for balanced scaling
#             - Fixed under-scaling issues for non-square images (e.g., widescreen formats)
...
#             - Marker sizes now scale proportionally: small maps = smaller markers, large maps = larger markers
#             - All elements (text, symbols, spacing, markers) now scale consistently with image size
#             - Fixed legend symbol proportions: reduced from 2rem to 1.3rem for better balance with text (1.3:1 ratio instead of 2:1)
#   v0.8.7.2 - Missing Data Filter and Zoom Consistency (Bug 2 + UX-1):
#             - Added MISSING_DATA_THRESHOLD constant (20% threshold)
#             - Implemented missing data filtering in process_oa_data() function
#             - Sites with >20% missing data are now filtered out before mapping
#             - Updated all legend labels from "Site not in use" to "Insufficient data"
#             - Fixed zoom level inconsistency: static images now use same padding as HTML maps (30px)
#             - Ensures markers fill screen consistently across both output formats
#             - Requires enriched data file with missing_no2 and missing_pm25 columns
#   v0.8.7.3 - Static Map Initialization and Zoom Fix (Bug fixes for UX-1):
#             - Fixed static map initialization: now includes sf_data_wgs84 like HTML map
#             - Static map now uses same leafletOptions (zoomDelta, zoomSnap) as HTML map
#             - Reduced padding from 30px to 5px for both maps (tighter fill, borough appears larger)
#             - Fixes issue where static images were zoomed out too far with vignette not extending to edges
#             - Both HTML and static maps now zoom identically and fill viewport/image better
#   v0.8.8 - Boundary Labels Control (Critical-1, 2025-10-16):
#             - Added show_boundary_labels boolean parameter to create_pollution_map() (default: FALSE)
#             - Modified add_boundary_polygons() to support configurable label visibility
#             - TRUE = borough boundary labels always visible on both interactive and static maps
#             - FALSE = no boundary labels (default behavior, maintains backward compatibility)
#             - Labels use consistent styling with noHide=TRUE for permanent visibility
#             - Threaded parameter through add_map_controls() to both HTML and static map generation paths
#             - Note: Hover-only labels not implemented due to layer order (markers cover polygons)
#   v0.8.7.4 - Bug 1 Fix: CSV File Path Handling (2025-10-15):
...
#             - Resolves inconsistency where CSV files required full paths while RData used DATA_PATH
...
load_data_file <- function(
  ...
  tryCatch(
    {
      switch(
        # TODO investigate why csv files don't seem to load correctly from the data folder btu rdata files do
        file_type,
...
apply_custom_layout_in_html <- function(
  ...
  image_dimensions = c(1200, 1200) # NEW: [width, height] for sizing
) {
...
create_generic_icons <- function(
  ...
  pollutant = NULL,
  scale_to_use = NULL,
  image_scale_factor = 1 # NEW: Scale factor for static image sizing
) {
...
add_layer <- function(
  ...
  scale_to_use = NULL,
  label_sizing = 1.0,
  image_scale_factor = 1.0 # NEW: Scale factor for static image marker sizing
) {
  ...
  icons <- create_generic_icons(
    ...
    pollutant,
    scale_to_use,
    image_scale_factor # NEW: Pass scale factor for marker sizing
  )
...
# TODO: unify HTML map creations and the static map creation functions so static maps can use the HTML banners and legends
...
create_pollution_map <- function(
  ...
  for (yr in unique(years_to_plot)) {
    ...
    html_map <- generate_map_layers(
      ...
      scale_to_use,
      environment(),
      1.0 # Interactive maps use default scale factor (no scaling)
    )
    ...
    if (image_export) {
      ...
      static_map <- generate_map_layers(
        ...
        scale_to_use,
        environment(),
        marker_scale_factor # NEW: Pass scale factor for marker sizing
      )
