...
# Version 0.8.7.2 - Missing Data Filter and Zoom Consistency Fix
  ...
  tryCatch(
    {
      ...
      extended_bbox <- c(
        original_bbox["xmin"] - 0.2 * width,
        original_bbox["ymin"] - 0.2 * height,
        original_bbox["xmax"] + 0.2 * width,
        original_bbox["ymax"] + 0.2 * height
      )
...
create_generic_icons <- function(
  ...
  base_shape_config <- switch(
    layer_type,
    "schools" = list(shape = 'cross', size = 8), # Base size for 1200x1200
    "dt_sites" = list(shape = 'circle', size = 15), # Base size for 1200x1200
    "bl_nodes" = list(shape = 'diamond', size = 15), # Base size for 1200x1200
...
add_map_controls <- function(
  ...
  # Fit bounds with consistent padding for both interactive and static maps
