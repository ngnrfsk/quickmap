...
.banner { background: %s; color: white; padding: 20px; text-align: center; font-size: 1.3em; flex-shrink: 0; }
.map-container { flex: 1; position: relative; min-height: 0; }
.map-container > div { height: 100%% !important; }
.legend { background: #f8f9fa; border-top: 2px solid #dee2e6; flex-shrink: 0; }
.legend-header { 
  padding: 15px 20px; 
  cursor: pointer; 
  user-select: none;
...
.legend-symbol { width: 20px; height: 20px; border-radius: 50%%; border: 2px solid rgba(0,0,0,0.2); }
@media (max-width: 480px) {
  .banner { padding: 8px; font-size: 1.1em; }
  .legend-items { flex-direction: column; gap: 8px; align-items: flex-start; }
}
</style>\n"
...
# Add full-width banner above map
add_full_width_banner <- function(
  map,
...
add_map_styling <- function(
  ...
  if (show_banner) {
    map <- add_full_width_banner(map, banner_text)
  }
...
create_pollution_map <- function(
  ...
  # TODO: Add default value for boroughs parameter to improve usability.
  # Currently required with no default, causing "argument 'boroughs' is missing" error.
  borough_sf <- tryCatch(
  ...
  # Apply custom styling (banner and footer)
  ...
  if (!is.null(output_file)) {
    ...
    # Apply custom layout with banner and external legend
    tryCatch(
      {
        apply_custom_layout(
          html_file = html_file,
