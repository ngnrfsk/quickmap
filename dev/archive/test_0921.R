# Test Suite for Step 1: Flexible Multi-data Source Loading (v0.9.2.1)
# Tests both old API (backward compatibility) and new API (data_sources/data_configs)
...
test1_old_api <- create_pollution_map(
  ...
  output_file = "test_0921_old_api_merton_no2.html",
...
test2_new_api <- create_pollution_map(
  ...
  years = 2023,
  colour_scale = "who_no2",
  output_file = "test_0921_new_api_merton_no2.html",
...
test3_partial <- create_pollution_map(
  ...
  years = 2023,
  colour_scale = "who_no2",
  output_file = "test_0921_new_api_partial.html",
...
test4_schools_only <- create_pollution_map(
  ...
  pollutant = "no2",
  colour_scale = "who_no2",
  output_file = "test_0921_old_api_schools_only.html",
...
test5_richmond <- create_pollution_map(
  ...
  output_file = "test_0921_new_api_richmond.html",
...
test6_pm25 <- create_pollution_map(
  ...
  output_file = "test_0921_old_api_pm25.html",
...
cat("  - test_0921_old_api_merton_no2.html (TEST 1)\n")
cat("  - test_0921_new_api_merton_no2.html (TEST 2 - should match TEST 1)\n")
cat("  - test_0921_new_api_partial.html (TEST 3)\n")
cat("  - test_0921_old_api_schools_only.html (TEST 4)\n")
cat("  - test_0921_new_api_richmond.html (TEST 5)\n")
cat("  - test_0921_old_api_pm25.html (TEST 6)\n")
