...
### Stack any number of sources — the `layers` list
...
at a glance. The year menu shows every year found in *any* layer; layers
without data for a year simply sit that year out.

![Three layers on one map: circles (tubes), diamonds (sensors), crosses (schools)](figures/layers-multilayer.png)

### Name a layer where you build it — `from_csv(name = )`
...
### Pick each layer's pollutant — `from_rdata(file, pollutant)`
...
### Skip the file: fetch data live — `from_openair()`

The [openair package](https://openair-project.github.io/openair/)
...
### Map a data frame you built yourself — `qm_layer()`

Any data frame with a site code, coordinates and a value column can be a
layer — this is how you map data QuickMap has never heard of. Required
columns: `code`, `lat`, `lon` and one value column:
...
### Each layer carries its own symbol — `qm_layer(shape = )`

Handy when two layers would otherwise look alike:

```{r choose-shape}
quickmap(
  list(
    from_csv("merton_dt_2018_2024.csv", name = "Council tubes"),  # circles
    qm_layer(my_sites, shape = "star", name = "My survey")        # stars
  ),
  boroughs = "Merton"
)
```
