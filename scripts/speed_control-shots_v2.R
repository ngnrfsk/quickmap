...
# Run after scripts/speed_demo-maps_v2.R.
# Rscript scripts/speed_control-shots_v2.R
#
...
b <- open_map("aq_maps/speed_merton-annual_v2.html", 1200, 900)
cat("annual speeds:", ev(b, "JSON.stringify(window.quickmapConfig.speeds)"), "\n")
shoot(b, "aq_maps/speed_card-annual-1x_v2.png")
press(b); press(b)
stopifnot(identical(label(b), "4×"))
shoot(b, "aq_maps/speed_card-annual-4x_v2.png")
press(b)
stopifnot(identical(label(b), "0.5×"))
shoot(b, "aq_maps/speed_card-annual-05x_v2.png")
b$close()

# 108 steps: the full set is still there
b <- open_map("aq_maps/speed_episode_v2.html", 1200, 900)
cat("episode speeds:", ev(b, "JSON.stringify(window.quickmapConfig.speeds)"), "\n")
press(b); press(b); press(b)
stopifnot(identical(label(b), "8×"))
shoot(b, "aq_maps/speed_card-episode-8x_v2.png")
press(b)
stopifnot(identical(label(b), "0.25×"))
shoot(b, "aq_maps/speed_card-episode-025x_v2.png")
b$close()

# a phone: the button goes, the rest of the card stays
b <- open_map("aq_maps/speed_merton-annual_v2.html", 390, 844)
shoot(b, "aq_maps/speed_card-mobile_v2.png", pad = 14)
b$close()

# the legend, with and without the indicator's maximum
b <- open_map("aq_maps/speed_merton-annual_v2.html", 1200, 900)
shoot(b, "aq_maps/speed_legend-mean_v2.png", ".legend-container", 10)
b$close()
b <- open_map("aq_maps/speed_merton-annual-max_v2.html", 1200, 900)
shoot(b, "aq_maps/speed_legend-mean-and-max_v2.png", ".legend-container", 10)
b$close()
